...
(defparam demo-app-host-port : Number
...
(defresource load-balancer
  (aws:elastic-load-balancing:load-balancer
   ...
   #:listeners [{ #:instance-port demo-app-host-port
...
(defresource service
  (aws:ecs:service
   #:cluster cluster
   #:role "ecsServiceRole"
   #:load-balancers [{ #:container-name "demo-app"
...
(defresource server-task
  (aws:ecs:task-definition
   #:family (fn:join [env "-demo-app"])
   #:container-definitions [{ #:name "demo-app"
                              #:image (fn:join ["114272735376.dkr.ecr.us-east-1.amazonaws.com/demo-app:" application-image-tag])
                              #:memory 128
                              #:port-mappings [{ #:container-port container-port
                                                 #:host-port demo-app-host-port }]}]))
...
(defresource domain
  (aws:route53:record-set
   ...
   #:name (fn:join [env "-demo-app.cjpowered.com"])
   #:type "A"))
